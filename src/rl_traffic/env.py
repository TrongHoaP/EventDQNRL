...
from src.rl_traffic.rewards import RewardInputs, build_reward
...
class SumoTrafficSignalEnv(gym.Env):
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        ...
        info = self._info(switched=switched, safety_decision=safety_decision, spawned=spawned)
    ...
    def _advance_control_interval(self) -> int:
        ...
        spawned = 0
        for _ in range(self.config.control.decision_interval_seconds):
            current_time = int(round(self.traci.simulation.getTime()))
            spawned += self.spawner.spawn_for_time(self.traci, current_time)
    ...
    def _info(self, switched: bool, safety_decision: Any | None, spawned: int) -> dict[str, Any]:
        assert self.traci is not None
        assert self.controller is not None
        total_queue, total_wait = self.controller._network_totals()
